    ...
    */
    ...
    fn convert_poll_message(
        ...
        if let Some(authority) = pseudo.authority {
            // rhino patch: be lenient about a malformed :authority. etcd v3.5
            // clients (e.g. the one bundled in k0s's kube-apiserver) set
            // :authority to "#initially=[http://host:port]", which is not a
            // valid URI authority. Go's grpc server passes it through; matching
            // that, we drop an unparseable authority (treat as absent) instead
            // of resetting the stream with PROTOCOL_ERROR. Valid authorities are
            // unaffected.
            match uri::Authority::from_maybe_shared(authority.clone().into_inner()) {
                Ok(a) => parts.authority = Some(a),
                Err(_) => {} // leave parts.authority = None
            }
        }
