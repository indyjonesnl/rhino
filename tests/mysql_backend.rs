//! MySQL backend integration tests.
//!
//! These tests require a running MySQL instance. Set the `RHINO_MYSQL_DSN`
//! environment variable to enable them:
//!
//!   RHINO_MYSQL_DSN="mysql://root:root@localhost/rhino_test" cargo test --test mysql_backend
//!
//! Each test truncates the kine table to ensure isolation.

use rhino::{Backend, BackendError, MysqlBackend, MysqlConfig};
use std::time::Duration;

fn get_dsn() -> Option<String> {
    std::env::var("RHINO_MYSQL_DSN").ok()
}

async fn test_backend() -> Option<MysqlBackend> {
    let dsn = get_dsn()?;
    ...
    let backend = MysqlBackend::new(config).await.unwrap();
    backend.start().await.unwrap();
    tokio::time::sleep(Duration::from_millis(50)).await;

    // Truncate for test isolation
    let pool = sqlx::mysql::MySqlPool::connect(&dsn).await.unwrap();
    sqlx::query("TRUNCATE TABLE kine")
        .execute(&pool)
        .await
        .unwrap();
    pool.close().await;
    ...
    let backend = MysqlBackend::new(config2).await.unwrap();
    backend.start().await.unwrap();
    tokio::time::sleep(Duration::from_millis(50)).await;
